---
title: GCC from Source
label: GCC from Source
order: 5
---
...
make -j16
```
Above, the `-j16` flag specifies that 16 jobs can run in parallel. Adapt this number to your system to maximize the parallelism. The build takes a long time.
...
At this point you should be able to build and run Warthog as described [here](./node/compiling#compiling). Note that after compiler upgrade, if you already generated the build directory, you should delete it and regenerate it using `meson setup build`.
